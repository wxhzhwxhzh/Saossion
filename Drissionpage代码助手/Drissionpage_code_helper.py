...
class SaoCode:
    ...
    def __init__(self, root):
        ...
        # 设置窗口标题和logo
        self.root.title(f"DrissionPage 代码生成助手 {version}-- 骚神库 ") 
    ...
    def create_widgets(self):
        ...
        ttk.Radiobutton(radio_frame, text="ChromiumPage", variable=mode_var, value="选项1").pack(side="left", padx=5)
        ...
        self.config['浏览器关联提示']=tk.StringVar(value="浏览器：自动")


        ttk.Label(browser_frame, text="关联浏览器:").pack(padx=5,side='left')        
        ttk.Radiobutton(browser_frame, text="自动选择浏览器", variable=self.config['浏览器关联'], value="自动",command=self.toggle_finger_button).pack(side="left", padx=5)
        ...
        self.config['浏览器上传文件选择'] = tk.StringVar(value='默认是空')

        browser_select=ttk.Label(upload_frame,textvariable=self.config['浏览器上传文件选择'])
        browser_select.pack(padx=5,side='left') 

        button = ttk.Button(upload_frame, text="选择上传文件...", command=self.open_file_dialog)
        button.pack()
        # -----------浏览器下载目录选择
...
class AboutTab():
    ...
    def run(self):
        ...
        ttk.Separator(about_frame,orient="horizontal").pack(fill="x")
        ttk.Label(about_frame, text=' 骚神库官网 ',underline=12,foreground='red').pack(pady=10,anchor='center')
        ttk.Label(about_frame, text='  \n\n    后续更新更精彩功能，顺手点个star⭐').pack(anchor='center')
